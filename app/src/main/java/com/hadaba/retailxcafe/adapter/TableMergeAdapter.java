...
//Item transfer and Merge   , same adapter
public class TableMergeAdapter extends BaseAdapter {
    ...
    public TableMergeAdapter(Context context, List<TableResponse> tableresponse, String identification) {
            ...
            if (identification.equals("fromtable")) {
                ...
                if (tables.getInvno().size() > 1) // invoice number more than 1 , just display for selection: >1
                {
                    ...
                    if (tables.getInvno().size() > 1) // invoice number more than 1 , just display for selection: >1
                    {
                        ...
                        InvoiceList.setOnItemClickListener((adapterView, view1, i, l) -> {
                            dialogView.cancel();
                            TableActivity.to_invoice_no =tables.getInvno().get(i) + "";

                        });
                        dialogView.show();
                    } else {
                        if (!tables.getInvno().isEmpty()) {
                            TableActivity.to_invoice_no = tables.getInvno().get(0);
                        } else {
